...
class ControllerStepOpenLoop(Node):
...
#### Service client functions ###################################################
    ...
    # Update tip (executed only after stage updated)
    def update_tip(self):
        self.tip = np.array([self.stage[0], self.stage[1], self.stage[2], 0.0, 0.0])
        self.tip_pose = np.array([self.tip[0], self.tip[1], self.tip[2], 1.0, 0.0, 0.0, 0.0])
        if (self.wait_finish is True):
            self.finish_insertion()
        elif (self.robot_idle is True):
            self.move_step()  
...
def main(args=None):
    ...
    controller_step_openloop = ControllerStepOpenLoop()

    executor = MultiThreadedExecutor()

    controller_step_openloop.get_logger().warn('***** Ready to INSERT - Open loop *****')
    controller_step_openloop.get_logger().warn('Use SPACE to signal each insertion step')

    try:
        rclpy.spin(controller_step_openloop, executor=executor)
    except KeyboardInterrupt:
        controller_step_openloop.get_logger().info('Keyboard interrupt, shutting down.\n')

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    controller_step_openloop.destroy_node()  
