...
def main(args=None):
    ...
    controller_sequence.get_logger().warn('***** Ready to INSERT - Sequence *****')
    controller_sequence.get_logger().warn('Use SPACE to signal each sequence step')

    try:
        rclpy.spin(controller_sequence, executor=executor)
    except KeyboardInterrupt:
        controller_sequence.get_logger().info('Keyboard interrupt, shutting down.\n')
